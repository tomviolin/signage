...
function outputMap(room) {
	...
	mapml = '<div class="rotating map" style="opacity:0"><ul data-role="listview" data-inset="true" data-theme="b" style="width:<?= $MAPWIDTH ?>px; float:left; margin-right: 0.5%; background: white">\n'
	      + '<li data-role="list-divider" data-theme="b">Room: '+roomid+'</li>\n<li class="imgrotator">\n';
	for (i = 0; i < roomMaps[roomid].length; ++i) {
		mapml += '  <img src="maps/'+roomMaps[roomid][i]+'" width="<?= $MAPWIDTH ?>" height="<?= $MAPHEIGHT ?>">\n';
	}
	mapml += "</li></ul></div>\n";
	//console.log(mapml);
	return mapml;
...
function outputFloor(floorpic) {
	mapml = '<div class="rotating optional" style="opacity:0"><ul data-role="listview" data-inset="true" class="maps" style="width:<?= $MAPWIDTH ?>px; float:left; margin-right: 0.5%;">\n'
	      + '<li data-role="list-divider">'+floorpic+'</li>\n<li class="imgrotator">\n';
...
function updaterooms(everything, nextMove) {
	...
	$.getJSON("getdata.php?now="+thisNow+"&limitevents="+numevents, function(data) {
		...
		// build room lists
		var rlistml = "";
		rlistml += outputRooms(data.imminentrooms, 'Classes/Meetings Starting Soon<br><b><-- LOOK FOR MAP</b>','images/doorflash.gif', '#f6c6c6','#000000');
		rlistml += outputRooms(data.juststartedrooms, 'Classes/Meetings Just Started<br><b><-- LOOK FOR MAP</b>','images/doorclosedflash.gif','#f6c6c6','#000000');
		rlistml += outputRooms(data.currentrooms, 'Classes/Meetings In Progress', 'images/doorsclosed.png', '#f6f6f6','#000000');
		rlistml += outputRooms(data.roomstoday, 'Coming Up Today', 'images/doorsopen.png');
		rlistml += outputRooms(data.roomstomorrow, 'Classes/Meetings Tomorrow, '+moment((thisNow+60*60*24)*1000).format('MMMM D Y'), 'images/doorsclosed.png', '#ffbd00','#000000');
		...
		if (mapml != ""){
			...
			needShort = false;
			// floors
			mapml += outputFloor("floor1.png");
			mapml += outputFloor("floor3.png");
		}
		...
		if (nextMove) {
			console.log("launching "+nextMove+" in 200ms.");
...
function initRotator() {
	console.log("initRotator()");
	thisRotator = 0;
	rotators = $("#rotator .rotating").css("opacity","0.0");
	$($("#rotator .rotating")[0]).css("opacity","1.0");
	...
	//});
	window.setTimeout("rotateNext()", 3000);
}
...
function rotateNext() {
	...
	if (thisRotator==0) {
		window.setTimeout("updaterooms(true)",500);
	}
...
function mapRotator() {
	...
	$("li.imgrotator").each(function(index) {
		...
		// just a 'flicker'
		$(imgs[1]).animate({opacity:0.0},300);
		$(imgs[1]).animate({opacity:1.0},300);
	});
}	
...
<div data-role='page' id="page" style="background: rgba(0,0,0,0);">
	<div data-role="header" xclass="ui-bar" data-theme="c" data-position="fixed" id="mainhead">
		<h1 style="font-size: 75%">UWM School of Freshwater Sciences Meeting & Event Information<span style="display: inline-block; width: 45%" id="clock"></span></h1>
	</div>
	<div id="content" data-role="content" style="background: rgba(0,0,0,0) !important;">
		...
		<div id="rotator" style="position:relative; top:0; left:0">
			<div class="rotating static short" style="position:absolute; top:0;left:0"><ul data-role="listview" data-inset="true" ><li data-role="list-divider" data-theme="b">UWM Freshwater Webcam</li><li>The UWM School of Freshwater Sciences webcam records 24/7 images of the Inner Harbor of Milwaukee.  The background imagery on this display shows the most recent complete hour of images compressed into 30 seconds.</li></ul></div> 
			...
			<div style="top:0; left:0; width: <?= $MAPWIDTH ?>px;">
