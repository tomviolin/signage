...
function insertEvents($icsFile, $psource='') {
	...
	while (true) {
		$x = "";
		$x = @file_get_contents($icsFile);
		if ($x == "") continue;
		break;
	}

	file_put_contents("/dev/shm/$source-$pid-tmp.ics", $x);
	system("ical2json /dev/shm/$source-$pid-tmp.ics");
	$ical   = json_decode(file_get_contents("/dev/shm/$source-$pid-tmp.json"));
	...
	for ($i = 0; $i < 10; ++$i) {
		try {
			...
			//echo "reading file $icsFile failed!\n";
			sleep(0.1);
			continue;
		}
	}
	...
	foreach ($events as $event) {
		...
		foreach ($event as $item => $value) {
			...
			foreach ($items as $itemv) {
				if ($itemv != $value) {
					$itemvs = explode("=",$itemv);
					if (count($itemvs)>1) $eva[$items[0].":".$itemvs[0]] = $itemvs[1];
				}
			}
		}
		if (isset($eva['DTSTART:VALUE'])) {
			if ($eva['DTSTART:VALUE'] == 'DATE') {
				$eva['DTSTART'] .= "T000000";
			}
		}
		$date = strtotime($eva['DTSTART']);
		$datepr = date('Y-m-d H:i:s',$date);
		if (isset($eva['DTEND:VALUE'])) {
			if ($eva['DTEND:VALUE'] == 'DATE') {
				$eva['DTEND'] .= "T000000";
			}
		}
		$end  = strtotime($eva['DTEND']);
		$endpr = date('Y-m-d H:i:s',$end);
		$title = fixicstext($eva['SUMMARY']);
		$body = fixicstext($eva['DESCRIPTION']);
		if (isset($eva['LOCATION'])) {
			$location=fixicstext($eva['LOCATION']);
		} else {
			$location="";
		}
		$categories=""; if (isset($eva['CATEGORIES'])) $categories=$event->CATEGORIES;
		$attachurl = "";
		...
		//print_r($eva);
		//echo "DATA: $date,$end,$title,substr($body,0,10),$location,$attachurl,$categories,$source,$url\n";
		$result = $st->execute();
	}
	//unlink("/dev/shm/$source-$pid-tmp.ics");
}

//insertEvents('http://uwm.edu/freshwater/events/?ical=1');
insertEvents('http://waterbase.uwm.edu/roar/roomcalendars/GLRF_ALL.ics', 'rooms');
insertEvents('https://calendar.google.com/calendar/ical/omohit5llfij2fnafsadkeihlc%40group.calendar.google.com/private-b83faceb660da1fd7e5793f57e96bdfc/basic.ics', 'scroller');
// SFS security notices calendar
insertEvents('https://outlook.office365.com/owa/calendar/8f91ecc82a9542be811db6c2bd6d4a50@uwm.edu/9a3caf2b9d4440bab6f8f90122f1c27c9813474339285178648/S-1-8-3600412639-52045316-1788038056-1369786580/reachcalendar.ics', 'security');

$lastmidnight = strtotime(date("Y-m-d 00:00:00",$now));

// immediate past room bookings
$justmissed=[];
$query = "select * from events where dtstart > $lastmidnight and dtstart < $now and dtend between $now-30*60 and $now and source = 'rooms' order by dtstart, dtend";
...
$juststartedrooms=[];
$query = "select * from events where dtstart > $lastmidnight and dtstart <= $now and dtstart between $now-20*60 and $now and dtend >= $now and source = 'rooms' order by dtstart,dtend";
$result = $db->query($query);
while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
...
$currentrooms=[];
$query = "select * from events where dtstart > $lastmidnight and dtstart <= $now and dtstart < $now-20*60 and dtend >= $now and source = 'rooms' order by dtstart,dtend";
$result = $db->query($query);
while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
	$currentrooms[]= $row;
}
...
$imminentrooms = [];
$query = "select * from events where dtstart between $now and $now+60*15 and source = 'rooms' order by dtstart,dtend";
$result = $db->query($query);
while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
	$imminentrooms[]= $row;
}
...
$query = "select * from events where dtstart > $now+60*15 and dtstart <= $thismidnight and source like 'rooms' order by dtstart,dtend";
$result = $db->query($query);
while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
	$roomstoday[] = $row;
}
...
$query = "select * from events where dtstart > $thismidnight and dtstart < $nextmidnight and source like 'rooms' order by dtstart,dtend";
$result = $db->query($query);
while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
	$roomstomorrow[] = $row;
}
...
$events =[];
$query="select * from events where source = 'security' order by dtstart, dtend limit ($limitevents)+0";
$result = $db->query($query);
while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
	$events[] = $row;
}
