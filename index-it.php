...
<style>
	...
	#announcements, #sliders, #roomlist {
		margin-top: 0px;
	}
	#announcements>li {
		padding-top: 0em;
	...
	div.desc { max-height: 6em; overflow-y: scroll; }
	div.desc div.brtag {
		height: 0.3em;
	}
...
function outputRooms(roomarray, sectionName, sectionIcon,sectionColor,sectionFontColor) {
	...
	if (roomarray.length>0) {
		...
		for (i = 0; i < roomarray.length; ++i) {
			x = roomarray[i];
			roomClass = '';
			if (moment(x.dtstart*1000).hour() < 8) {
				roomClass += ' before-hours';
			}
			if ((moment(x.dtend*1000).hour() == 17 &&
				moment(x.dtend*1000).minute() > 0) ||
				(moment(x.dtend*1000).hour() > 17)) {
				roomClass += ' after-hours';
			}
			
			html += '<li class="'+roomClass+'"><img src="'+sectionIcon+'" class="ui-li-icon" height=24><div style="display: inline-block; width: 8.4em; text-align:center;color: #887700">' + moment(x.dtstart*1000).format('h:mmA-') + moment(x.dtend*1000).format('h:mmA')+'</div> '+x.subject+'</li>\n';
		}
	}
	return (html);
}
...
function updaterooms(everything) {
	...
	$.getJSON("getdata-it.php?now="+thisNow+"&limitevents="+numevents, function(data) {
		// build room lists
		var html = "";
		...
		eventml='<li data-role="list-divider" data-theme="b">Security/IT Notices</li>';
		...
		if (bldg.length >= 1) {
			...
			bldghtml += '<li><h1>'+bldg[0].subject+'</h1><div class="desc">'+bldg[0].body+'</div></li>';
...
#sliders.ui-listview>.ui-li-static {
	white-space: normal;
	padding-left: 8em;
	padding-right: 8em;
	min-height: 5em;
...
#sliders.ui-listview>.ui-li-static div.datebox {
...
#sliders.ui-listview>.ui-li-static div.datebox .thedate {
	align-self: center;
	
}
#sliders.ui-listview>.ui-li-static div.qrbox {
...
#sliders.ui-listview>.ui-li-static div.qrbox img {
	align-self: center;
	
}

#sliders.ui-listview>.ui-li-static div.location {
	font-style: italic;
}
...
<div data-role='page' style="background: rgba(0,0,0,0);">
	...
	<div data-role="content" style="background: rgba(0,0,0,0) !important;">
		...
		<!-- upcoming events -->
		<ul data-role="listview" data-inset="true" id="sliders"  style="width: 68%; float: left; margin-right: 0.5%;">
